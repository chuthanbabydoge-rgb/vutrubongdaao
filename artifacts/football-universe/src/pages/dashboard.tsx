    ...
    <div className="container py-12 space-y-10">
      ...
      <div className="flex flex-col md:flex-row justify-between gap-6 items-start">
        <div className="space-y-2">
          <p className="text-muted-foreground font-mono text-sm uppercase tracking-wider">{greeting}, Commander</p>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {user.teamId && <Badge className="font-mono bg-primary/10 text-primary border-primary/30">Team Assigned</Badge>}
          </div>
        </div>
        <div className="flex gap-3">
          <Link href="/vr-gateway">
            <Button className="font-mono font-bold uppercase tracking-wider">
              <Rocket className="w-4 h-4 mr-2" /> Enter VR
            </Button>
          </Link>
          <Link href="/teams">
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { icon: Trophy, label: "Leagues", value: stats.totalLeagues, color: "text-yellow-400" },
            { icon: Users, label: "Teams", value: stats.totalTeams, color: "text-blue-400" },
            { icon: Star, label: "Players", value: stats.totalPlayers, color: "text-primary" },
            { icon: Activity, label: "Matches", value: stats.totalMatches, color: "text-green-400" },
      ...
      <div className="grid md:grid-cols-2 gap-10">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-black font-mono uppercase tracking-tighter flex items-center gap-2">
              <Activity className="w-5 h-5 text-primary" /> Recent Results
            </h2>
            <Link href="/matches">
          ...
          <div className="space-y-4">
            ...
            {!recentMatches?.length && (
              <Card className="p-8 text-center bg-card/30 border-border/30">
                <p className="text-muted-foreground font-mono text-sm">No recent matches</p>
              </Card>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-black font-mono uppercase tracking-tighter flex items-center gap-2">
              <Globe className="w-5 h-5 text-primary" /> Leagues
            </h2>
            <Link href="/leagues">
              <Button variant="ghost" size="sm" className="font-mono text-xs uppercase">
                View All <ArrowRight className="w-3 h-3 ml-1" />
              ...
              <Link key={league.id} href={`/leagues/${league.id}`}>
                <div className="group flex items-center justify-between p-4 rounded-lg border border-border/30 bg-card/30 hover:border-primary/40 hover:bg-card/60 transition-all cursor-pointer">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground font-mono">{league.region} · {league.teamCount} teams</div>
                  </div>
