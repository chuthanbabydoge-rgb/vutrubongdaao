...
function StatBar({ label, value, icon }: { label: string; value: number; icon: React.ReactNode }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="text-xs text-muted-foreground w-24 font-mono uppercase tracking-wider flex-shrink-0">{label}</div>
...
export default function PlayerDetail() {
  ...
  if (!player) return <div className="container py-12 text-center text-muted-foreground font-mono">Không tìm thấy cầu thủ.</div>;
    ...
    <div className="container py-12 max-w-4xl space-y-10">
      <Link href="/players">
        <Button variant="ghost" className="font-mono text-sm uppercase -ml-2">
          <ArrowLeft className="w-4 h-4 mr-2" /> Tất Cả Cầu Thủ
        </Button>
      </Link>
      ...
      <div className="rounded-2xl border border-border/40 bg-card/40 overflow-hidden">
        <div className="p-8 flex flex-col md:flex-row gap-8 items-start bg-gradient-to-br from-primary/5 to-transparent">
          <PlayerAvatar name={player.name} size={128} className="border-4 border-primary/30 shadow-lg" />
          <div className="flex-1 space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Badge variant="outline" className="font-mono">{player.age} tuổi</Badge>
            </div>
          </div>
          <div className="flex flex-col items-center gap-1">
            <div className={`text-6xl font-black font-mono ${ratingColor(player.rating ?? 0)}`}>{player.rating}</div>
            <div className="text-xs text-muted-foreground uppercase tracking-wider">Tổng Hợp</div>
          </div>
        </div>
        ...
        <div className="p-8 grid md:grid-cols-2 gap-8 border-t border-border/40">
          <div className="space-y-4">
            <h3 className="font-black font-mono uppercase tracking-tighter text-sm text-muted-foreground">Chỉ Số</h3>
            <StatBar label="Tốc Độ" value={player.pace ?? 0} icon={<Zap className="w-4 h-4" />} />
            <StatBar label="Sút Bóng" value={player.shooting ?? 0} icon={<Target className="w-4 h-4" />} />
            <StatBar label="Chuyền Bóng" value={player.passing ?? 0} icon={<ArrowRightLeft className="w-4 h-4" />} />
            <StatBar label="Rê Bóng" value={player.dribbling ?? 0} icon={<Star className="w-4 h-4" />} />
            <StatBar label="Phòng Thủ" value={player.defending ?? 0} icon={<Shield className="w-4 h-4" />} />
            <StatBar label="Thể Lực" value={player.physical ?? 0} icon={<Dumbbell className="w-4 h-4" />} />
          </div>

          <div className="space-y-4">
            <h3 className="font-black font-mono uppercase tracking-tighter text-sm text-muted-foreground">Thống Kê Mùa Giải</h3>
            <div className="grid grid-cols-3 gap-4">
              {[
                { label: "Bàn Thắng", value: player.goals ?? 0, color: "text-green-400" },
                { label: "Kiến Tạo", value: player.assists ?? 0, color: "text-blue-400" },
                { label: "Trận Đấu", value: player.matchesPlayed ?? 0, color: "text-primary" },
              ].map(stat => (
            ...
            {(player.matchesPlayed ?? 0) > 0 && (
              <div className="grid grid-cols-2 gap-4 text-sm font-mono text-muted-foreground">
                <div className="p-3 bg-card/30 rounded-lg">
                  <span className="text-xs uppercase tracking-wider block mb-1">Bàn/Trận</span>
                  <span className="text-foreground font-bold">{((player.goals ?? 0) / (player.matchesPlayed ?? 1)).toFixed(2)}</span>
                </div>
                <div className="p-3 bg-card/30 rounded-lg">
                  <span className="text-xs uppercase tracking-wider block mb-1">Đóng Góp/Trận</span>
