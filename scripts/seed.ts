...
const {
  leaguesTable, teamsTable, playersTable,
  matchesTable, standingsTable,
} = schema;
...
const KNOWN_PLAYERS: Record<string, string[]> = {
  ...
  "Villarreal CF":       ["Filip Jörgensen","Alfonso Pedraza","Pau Torres","Pervis Estupiñán","Yeremy Pino","Alex Baena","Manu Trigueros","Samuel Chukwueze","Samu Chukwueze","Pepe Reina","Gerard Moreno"],
  ...
  "RB Leipzig":          ["Peter Gulácsi","Benjamin Henrichs","Willi Orbán","Lukas Klostermann","David Raum","Xaver Schlager","Kevin Kampl","Konrad Laimer","Dani Olmo","Timo Werner","Lois Openda"],
  "Bayer 04 Leverkusen": ["Lukáš Hrádecký","Jeremie Frimpong","Odilon Kossounou","Jonathan Tah","Alejandro Grimaldo","Robert Andrich","Granit Xhaka","Florian Wirtz","Jonas Hofmann","Granit Xhaka","Victor Boniface"],
  "Eintracht Frankfurt": ["Kevin Trapp","Rasmus Kristensen","Tuta","Robin Koch","Philipp Max","Ellyes Skhiri","Mario Götze","Knauff Ansgar","Junior Dina Ebimbe","Hugo Ekitiké","Randal Kolo Muani"],
  ...
  "Al Ittihad Club":     ["Marcelo Grohe","Siyabonga Ngezana","Jota","Fabinho","N'Golo Kanté","Romarinho","Steven Gerrard","Karim Benzema","Riyad Mahrez","Eduardo Camavinga","Roberto Firmino"],
  "Al Ahli Saudi FC":    ["Edouard Mendy","Saud Abdulhamid","Roger Ibañez","Roberto Carlos","Buno","Riyad Mahrez","Gabri Veiga","Merih Demiral","Franck Kessié","Allan Saint-Maximin","Ivan Toney"],
};
...
function makePlayers(teamId: number, teamName: string): Omit<typeof playersTable.$inferInsert, "id">[] {
  const names = KNOWN_PLAYERS[teamName] ??
    POSITIONS.map((_, i) => `Player ${i + 1} (${teamName})`);

  return POSITIONS.map((pos, i) => {
    const isAttacker = ["ST","CAM","LW","RW","CF"].includes(pos);
...
async function seed() {
  ...
  for (const [leagueId, teams] of teamsByLeague) {
    ...
    for (let i = 0; i < teams.length; i++) {
      for (let j = i + 1; j < teams.length; j++) {
        ...
        let status: "finished" | "live" | "upcoming";
        ...
        if (daysOffset < -1) {
          ...
          homeScore = rnd(0, 5);
          awayScore = rnd(0, 4);
        } else if (daysOffset === 0 && Math.random() > 0.6) {
          status = "live";
          homeScore = rnd(0, 3);
          awayScore = rnd(0, 2);
          minute = rnd(1, 90);
        ...
        matchRows.push({
          ...
          awayScore,
          minute,
          venue: teams[i].stadiumName ?? undefined,
        });
      }
    }
  }
  ...
  // 5. Compute standings from finished matches
  type StandingStats = { played: number; won: number; drawn: number; lost: number; gf: number; ga: number; pts: number; form: string[] };
  const stats = new Map<string, StandingStats>();

  const key = (leagueId: number, teamId: number) => `${leagueId}:${teamId}`;

  for (const [leagueId, teams] of teamsByLeague) {
    for (const t of teams) {
      stats.set(key(leagueId, t.id), { played: 0, won: 0, drawn: 0, lost: 0, gf: 0, ga: 0, pts: 0, form: [] });
    }
  }

  for (const m of matchRows) {
    if (m.status !== "finished" || m.homeScore == null || m.awayScore == null) continue;
    const hk = key(m.leagueId, m.homeTeamId);
    const ak = key(m.leagueId, m.awayTeamId);
    const hs = stats.get(hk)!;
    const as_ = stats.get(ak)!;
    if (!hs || !as_) continue;
    ...
    if (m.homeScore > m.awayScore) {
      hs.won++; hs.pts += 3; hs.form.push("W");
      as_.lost++; as_.form.push("L");
    } else if (m.homeScore < m.awayScore) {
      as_.won++; as_.pts += 3; as_.form.push("W");
      hs.lost++; hs.form.push("L");
    } else {
      hs.drawn++; hs.pts++; hs.form.push("D");
      as_.drawn++; as_.pts++; as_.form.push("D");
    }
  }

  // Build standings rows sorted by points
  const standingRows: Omit<typeof standingsTable.$inferInsert, "id">[] = [];
  for (const [leagueId, teams] of teamsByLeague) {
    const sorted = [...teams].sort((a, b) => {
      const sa = stats.get(key(leagueId, a.id))!;
      const sb = stats.get(key(leagueId, b.id))!;
      return (sb?.pts ?? 0) - (sa?.pts ?? 0);
    });

    sorted.forEach((team, idx) => {
      const s = stats.get(key(leagueId, team.id)) ?? { played: 0, won: 0, drawn: 0, lost: 0, gf: 0, ga: 0, pts: 0, form: [] };
      standingRows.push({
        leagueId,
        teamId: team.id,
        position: idx + 1,
